...
@interface BLCImagesTableViewController () <BLCMediaTableViewCellDelegate, UIViewControllerTransitioningDelegate, BLCMediaFullScreenDelegate,  BLCCameraViewControllerDelegate, BLCImageLibraryViewControllerDelegate>
...
// Ask Steve about this - not sure why this is an NSString and why the kindOfChange is an int
...
- (void) observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (object == [BLCDataSource sharedInstance] && [keyPath isEqualToString:@"mediaItems"]) {
        ...
        if (kindOfChange == NSKeyValueChangeSetting) {
            ...
            // Convert this NSIndexSet to an NSArray of NSIndexPaths (which is what the table view animation methods require) - dbk ask Steve
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation andCell:(BLCMediaTableViewCell *)cell andImageView:(UIImageView *)imageView {
    ...
    if (itemsToShare.count > 0) {
        ...
        if (isPhone) {
         ...
         //   self.sharePopover.popoverContentSize = CGSizeMake(320, 568);
            [self.sharePopover setPopoverContentSize:self.view.frame.size animated:YES];
            [self.sharePopover presentPopoverFromRect:cell.frame inView:self.view permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
...
#pragma mark - BLCMediaFullScreenDelegate

//-(void)didShareMedia:(BLCMedia *)item fromController:(UIViewController *)controller:(BLCMedia *)item fromController:(UIViewController *)controller {
  //  [self didShareMedia:item fromController:self viewTarget:self.view];
 //  }
...
//-(void)didShareMedia(BLCMedia *)item fromController:(UIViewC)
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    // Talk to Steve more about this - would like to explain this a little better
    
    CGFloat keyboardY = CGRectGetMinY(keyboardFrameInViewCoordinates);
    CGFloat commentViewY = CGRectGetMinY(commentViewFrameInViewCoordinates);
    NSLog (@"Comment View Min Y is %f", commentViewY);
    NSLog (@"Keyboard Y = %f", keyboardY);
    ...
    NSLog (@"Comment View Final = %f", commentViewFinalY);
...
//    //On rotation, the HeightToScroll is not moving up enough
//    if (difference > 0) {
